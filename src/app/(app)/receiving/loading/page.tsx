    ...
    <main className="container mx-auto p-4 md:p-8">
             ...
             <Dialog open={isManageOpen} onOpenChange={setIsManageOpen}>
                <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
                    ...
                    <form onSubmit={handleMasterScan} className="flex-shrink-0">
                     ...
                     <Button variant="outline" onClick={() => setIsRemainingVolumesOpen(true)} className="mt-2 w-full justify-center">
        ...
        <Dialog open={isRemainingVolumesOpen} onOpenChange={setIsRemainingVolumesOpen}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Volumes Pendentes de Leitura</DialogTitle>
                    <DialogDescription>
                    ...
                    <div className="space-y-4 pr-4">
                        {remainingVolumes.length > 0 ? (
                            remainingVolumes.map(quoteInfo => (
                                <div key={quoteInfo.quoteCode}>
                                    <h4 className="font-semibold">{quoteInfo.destinatario} ({quoteInfo.quoteCode})</h4>
                                    <div className="flex flex-wrap gap-1 mt-2">
                                        {quoteInfo.missingVolumes.map(barcode => (
                                            <Badge key={barcode} variant="secondary">{barcode}</Badge>
                                        ))}
                                    </div>
                                </div>
                            ))
                            ...
                            <div className="flex flex-col items-center justify-center text-center text-muted-foreground py-8">
                                ...
                                <p className="font-semibold">Todos os volumes foram lidos!</p>
                            </div>
                        )}
                    </div>
                </ScrollArea>
