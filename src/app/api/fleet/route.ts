...
export async function POST(request: Request) {
  try {
    ...
    if (!vehicleData.plate || !vehicleData.brand || !vehicleData.model || !vehicleData.bodyType || !vehicleData.vehicleType || !vehicleData.tara) {
      return NextResponse.json({ message: "Placa, Marca, Modelo, Tipo de Carroceria, Tipo de Rodado e Tara são obrigatórios." }, { status: 400 });
    }
    ...
    const existingVehicle = await db.collection('fleet').findOne({ $or: [{ plate: vehicleData.plate }, { renavam: vehicleData.renavam }, { chassis: vehicleData.chassis }] });
    if (existingVehicle) {
