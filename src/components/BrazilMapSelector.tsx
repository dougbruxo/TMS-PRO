...
import { X, Info, ZoomIn, ZoomOut, RotateCcw, Route, AlertCircle, Sparkles } from 'lucide-react';
import { motion, useAnimation } from 'framer-motion';
import { BRAZIL_ADJACENCY, findShortestPath, isValidNextStep } from '@/lib/brazil-map-utils';
import { toast } from '@/hooks/use-toast';

interface BrazilMapSelectorProps {
  selectedUfs: string[];
  onToggleUf: (uf: string) => void;
  onSetUfs: (ufs: string[]) => void;
  ufInicio?: string;
  ufFim?: string;
}
...
export function BrazilMapSelector({ selectedUfs, onToggleUf, onSetUfs, ufInicio, ufFim }: BrazilMapSelectorProps) {
  ...
  const handleSuggestRoute = useCallback(() => {
    if (!ufInicio || !ufFim) {
      toast({ variant: 'destructive', title: 'Atenção', description: 'Defina a origem e o destino primeiro.' });
      return;
    }
    const path = findShortestPath(ufInicio, ufFim);
    if (path) {
      onSetUfs(path);
      toast({ title: 'Sugestão de Rota', description: `Rota sugerida: ${[ufInicio, ...path, ufFim].join(' → ')}` });
    } else {
      toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível encontrar uma rota automática.' });
    }
  }, [ufInicio, ufFim, onSetUfs]);

  const handleToggleWithValidation = (uf: string) => {
    ...
    if (!ufInicio || !ufFim) {
       toast({ variant:'destructive', title:'Atenção', description:'Defina a UF de início e fim primeiro.' });
       return;
    }

    const isValid = isValidNextStep(uf, selectedUfs, ufInicio, ufFim);
    if (!isValid) {
      toast({ 
        variant: 'destructive', 
        title: 'Trajeto Inválido', 
        description: `O estado ${uf} não faz divisa com o último estado selecionado (${selectedUfs[selectedUfs.length-1] || ufInicio}).`,
        action: <Button variant="outline" size="sm" onClick={() => onToggleUf(uf)}>Forçar Seleção</Button>
      });
      return;
    }
    onToggleUf(uf);
  ...
  const isSelected = (uf: string) => selectedUfs.includes(uf);
  const isBoundary = (uf: string) => uf === ufInicio || uf === ufFim;

  return (
    <div className="flex flex-col xl:flex-row gap-6 items-start">
      {/* Sidebar Sequence */}
      <div className="w-full xl:w-72 flex flex-col gap-4">
        <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-primary font-semibold">
              <Route className="h-4 w-4" />
              <h3 className="text-sm">Trajeto</h3>
            </div>
            <Button variant="ghost" size="sm" onClick={() => onSetUfs([])} className="h-7 text-[10px] uppercase font-bold text-muted-foreground hover:text-destructive">
              Limpar
            </Button>
          </div>
          ...
          <Button 
            ...
            variant="outline" 
            className="w-full h-9 gap-2 text-xs font-bold border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary group"
          >
            <Sparkles className="h-3 w-3 animate-pulse group-hover:scale-125 transition-transform" />
            Sugerir Rota Automática
          </Button>
          
          <div className="relative pl-6 space-y-4 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gradient-to-b before:from-primary before:via-emerald-500 before:to-primary/40">
            {/* Inicio */}
            <div className="relative flex flex-col">
              <div className="absolute -left-[21px] top-1 h-[14px] w-[14px] rounded-full bg-primary border-2 border-white shadow-sm z-10" />
              <div className={cn(
                "p-3 rounded-xl border transition-all",
                ufInicio ? "bg-white dark:bg-black/40 border-primary/20 shadow-sm" : "bg-muted/30 border-dashed border-muted text-muted-foreground italic"
              )}>
                <span className="text-[9px] uppercase font-bold tracking-widest opacity-60 block mb-0.5">Origem</span>
                <span className="font-bold text-sm">{ufInicio || "Defina acima"}</span>
              </div>
            </div>

            {/* Percurso */}
            {selectedUfs.map((uf, idx) => (
              <div key={uf} className="relative flex flex-col animate-in slide-in-from-left-2 duration-300">
                <div className="absolute -left-[21px] top-1 h-[14px] w-[14px] rounded-full bg-emerald-500 border-2 border-white shadow-sm z-10 flex items-center justify-center text-[7px] text-white font-bold">
                  {idx + 1}
                </div>
                <div className="p-3 rounded-xl border border-emerald-500/20 bg-emerald-50/40 dark:bg-emerald-500/5 flex items-center justify-between group shadow-sm">
                  <div>
                    <span className="text-[9px] uppercase font-bold tracking-widest text-emerald-600/60 block mb-0.5">Passagem</span>
                    <span className="font-bold text-sm text-emerald-700">{uf}</span>
                  </div>
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="h-6 w-6 text-emerald-600 hover:bg-emerald-100 dark:hover:bg-emerald-950 opacity-0 group-hover:opacity-100 transition-opacity"
            ...
            <div className="relative flex flex-col">
              ...
              <div className={cn(
                "p-3 rounded-xl border transition-all",
                ufFim ? "bg-white dark:bg-black/40 border-primary/20 shadow-sm" : "bg-muted/30 border-dashed border-muted text-muted-foreground italic"
              )}>
                <span className="text-[9px] uppercase font-bold tracking-widest opacity-60 block mb-0.5">Destino Final</span>
                <span className="font-bold text-sm">{ufFim || "Defina acima"}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {selectedUfs.length === 0 && (
          <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200/50 dark:border-blue-900/50 text-[11px] text-blue-700 dark:text-blue-400 leading-relaxed flex gap-3">
            <Info className="h-4 w-4 shrink-0 mt-0.5" />
            <p>O sistema sugere a rota automaticamente, mas você pode ajustar clicando nos estados do mapa respeitando as divisas.</p>
          </div>
        )}
      </div>

      {/* Map Area */}
      <div className="flex-1 w-full bg-slate-50 dark:bg-black/20 border-2 border-slate-200/60 dark:border-white/5 rounded-3xl p-4 shadow-inner relative overflow-hidden group min-h-[500px] flex items-center justify-center">
        {/* Map Controls */}
        <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-30">
          <Button variant="secondary" size="icon" onClick={handleZoomIn} className="rounded-full shadow-lg h-9 w-9"><ZoomIn className="h-4 w-4" /></Button>
          <Button variant="secondary" size="icon" onClick={handleZoomOut} className="rounded-full shadow-lg h-9 w-9"><ZoomOut className="h-4 w-4" /></Button>
          <Button variant="secondary" size="icon" onClick={resetMap} className="rounded-full shadow-lg h-9 w-9"><RotateCcw className="h-4 w-4" /></Button>
        </div>

        {/* Info Legend */}
        <div className="absolute top-6 right-6 flex flex-col gap-2 z-20">
           <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-white/90 dark:bg-slate-900/90 px-3 py-1.5 rounded-full shadow-sm border border-slate-200 dark:border-slate-800">
             <div className="h-2 w-2 rounded-full bg-primary shadow-[0_0_8px_rgba(59,130,246,0.5)]" /> Início / Fim
           </div>
           <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-white/90 dark:bg-slate-900/90 px-3 py-1.5 rounded-full shadow-sm border border-slate-200 dark:border-slate-800">
             <div className="h-2 w-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" /> Percurso
           </div>
        </div>
        ...
        >
          <svg 
            viewBox="0 0 450 460" 
            className="w-full h-auto max-h-[700px] drop-shadow-xl relative z-10 pointer-events-auto"
            preserveAspectRatio="xMidYMid meet"
          >
            {Object.entries(BRAZIL_STATES).map(([uf, state]) => {
              const selected = isSelected(uf);
              const boundary = isBoundary(uf);
              const sequenceIndex = selectedUfs.indexOf(uf);

              return (
                <g key={uf} className="cursor-pointer group/uf" onClick={() => !boundary && handleToggleWithValidation(uf)}>
                  {/* State Path */}
                  <path
                    d={state.d}
                    className={cn(
                      "transition-all duration-300 stroke-[1px] stroke-slate-400 dark:stroke-slate-600",
                      boundary 
                        ? "fill-primary stroke-primary/30 stroke-[2px]" 
                        : selected
                          ? "fill-emerald-500 stroke-emerald-600"
                          : "fill-white dark:fill-slate-900 hover:fill-slate-100 dark:hover:fill-slate-800"
                    )}
                  />
                  {/* Optional Circle (for smaller states) */}
                  {state.circleD && (
                    <path
                      d={state.circleD}
                      className={cn(
                  ...
                  <text
                    ...
                    textAnchor="middle"
                    className={cn(
                      "font-bold text-[9px] pointer-events-none select-none transition-all duration-300",
                      boundary || selected ? "fill-white" : "fill-slate-500 group-hover/uf:fill-primary"
                    )}
                  >
                    {uf}
                  </text>
                  
                  {/* Sequence Indicator Bubble */}
                  {selected && (
                    <g className="animate-in zoom-in duration-300">
                      <circle
                        ...
                        cy={state.labelY - 8}
                        r="6"
                        className="fill-emerald-700 stroke-white stroke-[0.5px]"
                      />
                      <text
                        x={state.labelX + 8}
                        y={state.labelY - 6.5}
