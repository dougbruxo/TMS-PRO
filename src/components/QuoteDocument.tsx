            ...
            <>
              ...
              <div className="section" style={{ border: '1px solid #e2e8f0', borderRadius: '0.5rem', padding: '0.75rem', backgroundColor: '#f8fafc', marginBottom: '0.75rem' }}>
                ...
                <div className="grid-cols-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0 3rem', fontSize: '0.75rem' }}>
                  <dl className="info-item" style={{ display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0', borderBottom: '1px solid #e2e8f0' }}><dt style={{ color: '#4a5568' }}>Frete Base:</dt> <dd style={{ fontWeight: 500, textAlign: 'right' }}>{formatCurrency((quote.valorFinal || 0) - (quote.extras?.reduce((acc, ex) => acc + (ex.total || 0), 0) || 0) - acrescimoRegionalValor - (quote.adicional || 0) - (quote.adicionalProduto || 0) - (quote.taxaDificuldade || 0))}</dd></dl>
                  <dl className="info-item" style={{ display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0', borderBottom: '1px solid #e2e8f0' }}><dt style={{ color: '#4a5568' }}>Km Rodado ({(quote.kmExcedente || 0).toFixed(0)} km):</dt> <dd style={{ fontWeight: 500, textAlign: 'right' }}>{formatCurrency(quote.adicional)}</dd></dl>
                  ...
                  <dl className="info-item" style={{ display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0', borderBottom: '1px solid #e2e8f0' }}><dt style={{ color: '#4a5568' }}>Custo Operacional Regional ({quote.regiao} - {((quote.acrescimoRegional || 0) * 100).toFixed(0)}%):</dt> <dd style={{ fontWeight: 500, textAlign: 'right' }}>{formatCurrency(acrescimoRegionalValor)}</dd></dl>
                  ...
                  <dl className="info-item" style={{ display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0', borderBottom: '1px solid #e2e8f0' }}><dt style={{ color: '#4a5568' }}>ICMS ({(quote.icmsAliquota || 0).toFixed(2)}%):</dt> <dd style={{ fontWeight: 500, textAlign: 'right' }}>{formatCurrency(quote.icmsValor)}</dd></dl>
                  {quote.extras && quote.extras.length > 0 && quote.extras.map((extra, idx) => (
                    <dl key={idx} className="info-item" style={{ display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0', borderBottom: '1px solid #e2e8f0' }}>
                      <dt style={{ color: '#4a5568' }}>{extra.name} ({extra.quantity}x {formatCurrency(extra.unitValue)}):</dt>
                      <dd style={{ fontWeight: 500, textAlign: 'right' }}>{formatCurrency(extra.total)}</dd>
                    </dl>
                  ))}
                </div>
              </div>
